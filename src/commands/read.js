...
const read = async (txid, network = MAINNET) => {
	const response = await fetch(
		`https://api.run.network/v1/${network.short}/rawtx/${txid}`
	);
	if (!response.ok) {
	...
	const bOutput = tx.outputs.find((o) => {
		return (
			o.script &&
			o.script.chunks[2] &&
			Buffer.from("19HxigV4QyBv3tHpQVcUEQyq1pzZVdoAut").equals(
				o.script.chunks[2].buf
			)
		);
	});
	...
	return new BFile(
		...
		bOutput.script.chunks[7] ? bOutput.script.chunks[7].buf.toString() : "",
		bOutput.script.chunks[8] ? bOutput.script.chunks[8].buf.toString() : "",
		bOutput.script.chunks[9] ? bOutput.script.chunks[9].buf.toString() : ""
	);
};
